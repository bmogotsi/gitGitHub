...
try:
    ...
    if soup_0.contents != []:
        ...
        if standing_table != []:
            ...
            links = [base_url+l.get("href") for l in elements] #get all links from tag "a" with "href" attribute 
            ...
            if response.status_code == 200 and debugYes==True:
                save_epl="Exception_Handling_ExceptionClass_doTeams.txt"
                createToTextFile(save_epl,'w')
                for link in links:
                    writeToTextFile(save_epl,str(link))
                    team_url=link
                    try:
                        data = rate_limited_request(team_url) #@@@ 429 Too Many Requests: delay # response (returns html bytes)
                        if data.status_code != 200:
                            raise ScraperException("Failed to extract team_url data", team_url) # This code first Invokes the CLASS then the below "ScraperException as e:"
                    except ScraperException as e:
                        logging.error(f"Scraping error. team_url get(url)  {team_url}")
                        # logger.error(f"Exception Error: response = rate_limited_request({url})  : {str(e)}")
                        sys.exit()
                    
                    matches =pd.read_html(data.text, match="Scores & Fixtures")
                    writeToTextFile(save_epl,str("\n\n"))
                    writeToTextFile(save_epl,str(matches[0].head))
                    #break
